...
(is (= 200 (:status refresh-response))
    "Refresh token exchange should return 200 OK")
(is (contains? (:body refresh-response) :access_token)
    "Should contain new access_token")
(is (contains? (:body refresh-response) :refresh_token)
    "Should contain new refresh_token")
(is (= "Bearer" (:token_type (:body refresh-response)))
    "Token type should be Bearer")
(is (= 3600 (:expires_in (:body refresh-response)))
...
(let [access-token-2 (:access_token (:body refresh-response))
  ...
  (is (not= access-token-1 access-token-2)
      "Access tokens should be different after refresh")
  (is (not= refresh-token-value refresh-token-2)
  ...
  (let [invalid-refresh-request {:db (db)
                                 ...
                                 {:form {:grant_type "refresh_token"
                                         ...
                                         :client_id oauth-client-id
                                         :client_secret oauth-client-secret}}}
        invalid-refresh-response (oauth/POST-exchange-token
                                  invalid-refresh-request)]
    (is (= 400 (:status invalid-refresh-response))
        "Used refresh token should be invalid")
    (is (= "invalid_grant" (:error (:body invalid-refresh-response)))
        "Should return invalid_grant error"))
  ...
  (let [invalid-client-request {:db (db)
        ...
        invalid-client-response (oauth/POST-exchange-token
                                 invalid-client-request)]
    (is (= 400 (:status invalid-client-response))
        "Invalid client secret should be rejected")
    (is (= "invalid_client" (:error (:body invalid-client-response)))
        "Should return invalid_client error")))
